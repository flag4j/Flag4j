...
The `<pre>...</pre>` block will be discarded when making the conversion.
...
For more complicated $ \LaTeX $ Replacements, users may want to specify <em>exactly</em> what
the $ \LaTeX $ should be replacing the HTML.
To do this, the `<span class="latex-replace">...</span>` block is used.
...
be included for the equation to be replaced. If there is not a latex equation following this, the
tag will be ignored.
...
We do not want the $ \LaTeX $ equation to be rendered in an IDE as raw text.
To avoid this, we place the equation inside a special HTML comment.
The comment must begin with `<!-- LATEX:`.
```java
/**
 * <!-- LATEX:
 * $$ \begin{bmatrix}
...
block for it to be used (white space and newlines are allowed between the two). 
...
HTML equation. Once the Javadocs are built, parsed, and deployed to the [Flag4j API website](https://flag4j.github.io/Flag4j/),
